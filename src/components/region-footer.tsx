...
export function RegionFooter({ config }: RegionFooterProps) {
  const companyLinks = [
    { label: "About Us", href: config.aboutPath },
    ...(config.caseStudyPath ? [{ label: "Case Studies", href: config.caseStudyPath }] : []),
    ...
    <footer className="bg-navy border-t border-white/10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div>
            <Link href={`/${config.cc}/`} className="inline-block mb-4">
              <Image
        ...
        <div className="mt-12 border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/40">
            &copy; {new Date().getFullYear()} Carelabs {config.countryName}. All rights reserved.
          </p>
