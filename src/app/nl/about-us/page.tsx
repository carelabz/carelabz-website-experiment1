...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("nl");
  return {
    title: page?.metaTitle ?? "About Carelabs | Netherlands Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and NEN 1010 compliance across Netherlands.",
    alternates: {
      canonical: "https://carelabz.com/nl/about-us/",
      languages: {
        "en-NL": "https://carelabz.com/nl/about-us/",
        "x-default": "https://carelabz.com/nl/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Netherlands",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Netherlands.",
      url: "https://carelabz.com/nl/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function NLAboutPage() {
  const page = await getAboutPage("nl");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Netherlands.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/nl/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Netherlands",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Netherlands.",
      "en-NL"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/nl/" },
      { name: "About", url: "https://carelabz.com/nl/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Netherlands"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Netherlands experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/nl/contact-us/"}
