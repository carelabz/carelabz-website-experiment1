...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("nl");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Netherlands | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and NEN 1010 compliance services across Netherlands.",
    alternates: {
      canonical: "https://carelabz.com/nl/contact-us/",
      languages: {
        "en-NL": "https://carelabz.com/nl/contact-us/",
        "x-default": "https://carelabz.com/nl/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Netherlands",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Netherlands.",
      url: "https://carelabz.com/nl/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function NLContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "NL");
  // On the Netherlands contact page the banner must always say Netherlands
  // regardless of the visitor's IP — the services ARE in Netherlands.
  const countryName = "Netherlands";

  const [page, services] = await Promise.all([
    getContactPage("nl"),
    getServicesByRegion("nl"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Netherlands team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/nl/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Netherlands",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Netherlands.",
      "en-NL"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/nl/" },
      { name: "Contact", url: "https://carelabz.com/nl/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Netherlands Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Netherlands team is available to respond to emergency compliance and
