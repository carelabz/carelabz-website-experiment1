...
const config = COUNTRY_CONFIGS["sa"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("sa");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Saudi Arabia | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and SBC 401 compliance services across Saudi Arabia.",
    alternates: {
      canonical: "https://carelabz.com/sa/contact-us/",
      languages: {
        "en-SA": "https://carelabz.com/sa/contact-us/",
        "x-default": "https://carelabz.com/sa/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Saudi Arabia",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Saudi Arabia.",
      url: "https://carelabz.com/sa/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SAContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "SA");
  // On the Saudi Arabia contact page the banner must always say Saudi Arabia
  // regardless of the visitor's IP — the services ARE in Saudi Arabia.
  const countryName = "Saudi Arabia";

  const [page, services] = await Promise.all([
    getContactPage("sa"),
    getServicesByRegion("sa"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Saudi Arabian team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "sa", countryName: "Saudi Arabia", countryIso2: "SA", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/sa/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Saudi Arabia",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Saudi Arabia.",
      "en-SA"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/sa/" },
      { name: "Contact", url: "https://carelabz.com/sa/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Saudi Arabia Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Saudi Arabian team is available to respond to emergency compliance and
