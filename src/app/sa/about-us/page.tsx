...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("sa");
  return {
    title: page?.metaTitle ?? "About Carelabs | Saudi Arabian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and SBC 401 compliance across Saudi Arabia.",
    alternates: {
      canonical: "https://carelabz.com/sa/about-us/",
      languages: {
        "en-SA": "https://carelabz.com/sa/about-us/",
        "x-default": "https://carelabz.com/sa/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Saudi Arabia",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Saudi Arabia.",
      url: "https://carelabz.com/sa/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SAAboutPage() {
  const page = await getAboutPage("sa");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Saudi Arabia.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/sa/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Saudi Arabia",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Saudi Arabia.",
      "en-SA"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/sa/" },
      { name: "About", url: "https://carelabz.com/sa/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Saudi Arabia"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Saudi Arabian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/sa/contact-us/"}
