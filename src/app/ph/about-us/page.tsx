...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("ph");
  return {
    title: page?.metaTitle ?? "About Carelabs | Philippines Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and Philippine Electrical Code compliance across Philippines.",
    alternates: {
      canonical: "https://carelabz.com/ph/about-us/",
      languages: {
        "en-PH": "https://carelabz.com/ph/about-us/",
        "x-default": "https://carelabz.com/ph/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Philippines",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Philippines.",
      url: "https://carelabz.com/ph/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PHAboutPage() {
  const page = await getAboutPage("ph");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Philippines.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/ph/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Philippines",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Philippines.",
      "en-PH"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ph/" },
      { name: "About", url: "https://carelabz.com/ph/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Philippines"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Philippines experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/ph/contact-us/"}
