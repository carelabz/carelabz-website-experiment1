...
const config = COUNTRY_CONFIGS["ph"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("ph");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Philippines | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and Philippine Electrical Code compliance services across Philippines.",
    alternates: {
      canonical: "https://carelabz.com/ph/contact-us/",
      languages: {
        "en-PH": "https://carelabz.com/ph/contact-us/",
        "x-default": "https://carelabz.com/ph/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Philippines",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Philippines.",
      url: "https://carelabz.com/ph/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PHContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "PH");
  // On the Philippines contact page the banner must always say Philippines
  // regardless of the visitor's IP — the services ARE in Philippines.
  const countryName = "Philippines";

  const [page, services] = await Promise.all([
    getContactPage("ph"),
    getServicesByRegion("ph"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Philippines team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "ph", countryName: "Philippines", countryIso2: "PH", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/ph/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Philippines",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Philippines.",
      "en-PH"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ph/" },
      { name: "Contact", url: "https://carelabz.com/ph/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Philippines Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Philippines team is available to respond to emergency compliance and
