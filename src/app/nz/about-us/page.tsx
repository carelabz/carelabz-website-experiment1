...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("nz");
  return {
    title: page?.metaTitle ?? "About Carelabs | New Zealand Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and AS/NZS 3000 compliance across New Zealand.",
    alternates: {
      canonical: "https://carelabz.com/nz/about-us/",
      languages: {
        "en-NZ": "https://carelabz.com/nz/about-us/",
        "x-default": "https://carelabz.com/nz/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants New Zealand",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in New Zealand.",
      url: "https://carelabz.com/nz/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function NZAboutPage() {
  const page = await getAboutPage("nz");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across New Zealand.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/nz/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants New Zealand",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in New Zealand.",
      "en-NZ"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/nz/" },
      { name: "About", url: "https://carelabz.com/nz/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs New Zealand"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our New Zealand experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/nz/contact-us/"}
