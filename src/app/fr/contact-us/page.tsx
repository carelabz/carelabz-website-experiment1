...
const config = COUNTRY_CONFIGS["fr"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("fr");
  return {
    title: page?.metaTitle ?? "Contact Carelabs France | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and NF C 15-100 compliance services across France.",
    alternates: {
      canonical: "https://carelabz.com/fr/contact-us/",
      languages: {
        "en-FR": "https://carelabz.com/fr/contact-us/",
        "x-default": "https://carelabz.com/fr/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation France",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in France.",
      url: "https://carelabz.com/fr/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function FRContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "FR");
  // On the France contact page the banner must always say France
  // regardless of the visitor's IP — the services ARE in France.
  const countryName = "France";

  const [page, services] = await Promise.all([
    getContactPage("fr"),
    getServicesByRegion("fr"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our France team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "fr", countryName: "France", countryIso2: "FR", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/fr/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation France",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in France.",
      "en-FR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/fr/" },
      { name: "Contact", url: "https://carelabz.com/fr/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs France Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our France team is available to respond to emergency compliance and
