...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("tw");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Taiwan | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and User Electricity Supply Regulations compliance services across Taiwan.",
    alternates: {
      canonical: "https://carelabz.com/tw/contact-us/",
      languages: {
        "en-TW": "https://carelabz.com/tw/contact-us/",
        "x-default": "https://carelabz.com/tw/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Taiwan",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Taiwan.",
      url: "https://carelabz.com/tw/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function TWContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "TW");
  // On the Taiwan contact page the banner must always say Taiwan
  // regardless of the visitor's IP — the services ARE in Taiwan.
  const countryName = "Taiwan";

  const [page, services] = await Promise.all([
    getContactPage("tw"),
    getServicesByRegion("tw"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Taiwan team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/tw/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Taiwan",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Taiwan.",
      "en-TW"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/tw/" },
      { name: "Contact", url: "https://carelabz.com/tw/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Taiwan Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Taiwan team is available to respond to emergency compliance and
