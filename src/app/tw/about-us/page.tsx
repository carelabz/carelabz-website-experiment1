...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("tw");
  return {
    title: page?.metaTitle ?? "About Carelabs | Taiwan Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and User Electricity Supply Regulations compliance across Taiwan.",
    alternates: {
      canonical: "https://carelabz.com/tw/about-us/",
      languages: {
        "en-TW": "https://carelabz.com/tw/about-us/",
        "x-default": "https://carelabz.com/tw/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Taiwan",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Taiwan.",
      url: "https://carelabz.com/tw/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function TWAboutPage() {
  const page = await getAboutPage("tw");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Taiwan.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/tw/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Taiwan",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Taiwan.",
      "en-TW"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/tw/" },
      { name: "About", url: "https://carelabz.com/tw/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Taiwan"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Taiwan experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/tw/contact-us/"}
