...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("ie");
  return {
    title: page?.metaTitle ?? "About Carelabs | Ireland Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and I.S. 10101:2020 compliance across Ireland.",
    alternates: {
      canonical: "https://carelabz.com/ie/about-us/",
      languages: {
        "en-IE": "https://carelabz.com/ie/about-us/",
        "x-default": "https://carelabz.com/ie/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Ireland",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Ireland.",
      url: "https://carelabz.com/ie/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function IEAboutPage() {
  const page = await getAboutPage("ie");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Ireland.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/ie/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Ireland",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Ireland.",
      "en-IE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ie/" },
      { name: "About", url: "https://carelabz.com/ie/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Ireland"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Ireland experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/ie/contact-us/"}
