...
const config = COUNTRY_CONFIGS["ie"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("ie");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Ireland | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and I.S. 10101:2020 compliance services across Ireland.",
    alternates: {
      canonical: "https://carelabz.com/ie/contact-us/",
      languages: {
        "en-IE": "https://carelabz.com/ie/contact-us/",
        "x-default": "https://carelabz.com/ie/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Ireland",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Ireland.",
      url: "https://carelabz.com/ie/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function IEContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "IE");
  // On the Ireland contact page the banner must always say Ireland
  // regardless of the visitor's IP — the services ARE in Ireland.
  const countryName = "Ireland";

  const [page, services] = await Promise.all([
    getContactPage("ie"),
    getServicesByRegion("ie"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Ireland team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "ie", countryName: "Ireland", countryIso2: "IE", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/ie/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Ireland",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Ireland.",
      "en-IE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ie/" },
      { name: "Contact", url: "https://carelabz.com/ie/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Ireland Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Ireland team is available to respond to emergency compliance and
