...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("fi");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Finland | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and SFS 6000 compliance services across Finland.",
    alternates: {
      canonical: "https://carelabz.com/fi/contact-us/",
      languages: {
        "en-FI": "https://carelabz.com/fi/contact-us/",
        "x-default": "https://carelabz.com/fi/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Finland",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Finland.",
      url: "https://carelabz.com/fi/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function FIContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "FI");
  // On the Finland contact page the banner must always say Finland
  // regardless of the visitor's IP — the services ARE in Finland.
  const countryName = "Finland";

  const [page, services] = await Promise.all([
    getContactPage("fi"),
    getServicesByRegion("fi"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Finland team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/fi/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Finland",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Finland.",
      "en-FI"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/fi/" },
      { name: "Contact", url: "https://carelabz.com/fi/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Finland Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Finland team is available to respond to emergency compliance and
