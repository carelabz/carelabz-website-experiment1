...
const config = COUNTRY_CONFIGS["th"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("th");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Thailand | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and EIT Standard 2001-56 compliance services across Thailand.",
    alternates: {
      canonical: "https://carelabz.com/th/contact-us/",
      languages: {
        "en-TH": "https://carelabz.com/th/contact-us/",
        "x-default": "https://carelabz.com/th/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Thailand",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Thailand.",
      url: "https://carelabz.com/th/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function THContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "TH");
  // On the Thailand contact page the banner must always say Thailand
  // regardless of the visitor's IP — the services ARE in Thailand.
  const countryName = "Thailand";

  const [page, services] = await Promise.all([
    getContactPage("th"),
    getServicesByRegion("th"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Thailand team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "th", countryName: "Thailand", countryIso2: "TH", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/th/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Thailand",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Thailand.",
      "en-TH"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/th/" },
      { name: "Contact", url: "https://carelabz.com/th/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Thailand Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Thailand team is available to respond to emergency compliance and
