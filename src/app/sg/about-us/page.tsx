...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("sg");
  return {
    title: page?.metaTitle ?? "About Carelabs | Singapore Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and SS 638 compliance across Singapore.",
    alternates: {
      canonical: "https://carelabz.com/sg/about-us/",
      languages: {
        "en-SG": "https://carelabz.com/sg/about-us/",
        "x-default": "https://carelabz.com/sg/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Singapore",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Singapore.",
      url: "https://carelabz.com/sg/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SGAboutPage() {
  const page = await getAboutPage("sg");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Singapore.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/sg/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Singapore",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Singapore.",
      "en-SG"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/sg/" },
      { name: "About", url: "https://carelabz.com/sg/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Singapore"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Singapore experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/sg/contact-us/"}
