...
const config = COUNTRY_CONFIGS["sg"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("sg");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Singapore | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and SS 638 compliance services across Singapore.",
    alternates: {
      canonical: "https://carelabz.com/sg/contact-us/",
      languages: {
        "en-SG": "https://carelabz.com/sg/contact-us/",
        "x-default": "https://carelabz.com/sg/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Singapore",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Singapore.",
      url: "https://carelabz.com/sg/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SGContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "SG");
  // On the Singapore contact page the banner must always say Singapore
  // regardless of the visitor's IP — the services ARE in Singapore.
  const countryName = "Singapore";

  const [page, services] = await Promise.all([
    getContactPage("sg"),
    getServicesByRegion("sg"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Singapore team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "sg", countryName: "Singapore", countryIso2: "SG", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/sg/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Singapore",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Singapore.",
      "en-SG"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/sg/" },
      { name: "Contact", url: "https://carelabz.com/sg/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Singapore Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Singapore team is available to respond to emergency compliance and
