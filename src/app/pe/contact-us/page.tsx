...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("pe");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Peru | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and RM 111-2013-MEM compliance services across Peru.",
    alternates: {
      canonical: "https://carelabz.com/pe/contact-us/",
      languages: {
        "en-PE": "https://carelabz.com/pe/contact-us/",
        "x-default": "https://carelabz.com/pe/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Peru",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Peru.",
      url: "https://carelabz.com/pe/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PEContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "PE");
  // On the Peru contact page the banner must always say Peru
  // regardless of the visitor's IP — the services ARE in Peru.
  const countryName = "Peru";

  const [page, services] = await Promise.all([
    getContactPage("pe"),
    getServicesByRegion("pe"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Peru team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/pe/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Peru",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Peru.",
      "en-PE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/pe/" },
      { name: "Contact", url: "https://carelabz.com/pe/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Peru Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Peru team is available to respond to emergency compliance and
