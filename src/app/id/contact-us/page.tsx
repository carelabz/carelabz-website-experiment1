...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: page?.metaTitle ?? "Contact Carelabs Indonesia | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and PUIL 2011 compliance services across Indonesia.",
    alternates: {
      canonical: "https://carelabz.com/id/contact-us/",
      languages: {
        "en-ID": "https://carelabz.com/id/contact-us/",
        "x-default": "https://carelabz.com/id/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Indonesia",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Indonesia.",
      url: "https://carelabz.com/id/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function IDContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "ID");
  // On the Indonesia contact page the banner must always say Indonesia
  // regardless of the visitor's IP — the services ARE in Indonesia.
  const countryName = "Indonesia";

  const [page, services] = await Promise.all([
    getContactPage("id"),
    getServicesByRegion("id"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Indonesian team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/id/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Indonesia",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Indonesia.",
      "en-ID"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/id/" },
      { name: "Contact", url: "https://carelabz.com/id/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Indonesia Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Indonesian team is available to respond to emergency compliance and
