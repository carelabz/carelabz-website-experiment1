...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("dk");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Denmark | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and DS/HD 60364 compliance services across Denmark.",
    alternates: {
      canonical: "https://carelabz.com/dk/contact-us/",
      languages: {
        "en-DK": "https://carelabz.com/dk/contact-us/",
        "x-default": "https://carelabz.com/dk/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Denmark",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Denmark.",
      url: "https://carelabz.com/dk/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function DKContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "DK");
  // On the Denmark contact page the banner must always say Denmark
  // regardless of the visitor's IP — the services ARE in Denmark.
  const countryName = "Denmark";

  const [page, services] = await Promise.all([
    getContactPage("dk"),
    getServicesByRegion("dk"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Denmark team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/dk/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Denmark",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Denmark.",
      "en-DK"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/dk/" },
      { name: "Contact", url: "https://carelabz.com/dk/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Denmark Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Denmark team is available to respond to emergency compliance and
