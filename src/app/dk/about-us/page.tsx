...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("dk");
  return {
    title: page?.metaTitle ?? "About Carelabs | Denmark Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and DS/HD 60364 compliance across Denmark.",
    alternates: {
      canonical: "https://carelabz.com/dk/about-us/",
      languages: {
        "en-DK": "https://carelabz.com/dk/about-us/",
        "x-default": "https://carelabz.com/dk/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Denmark",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Denmark.",
      url: "https://carelabz.com/dk/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function DKAboutPage() {
  const page = await getAboutPage("dk");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Denmark.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/dk/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Denmark",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Denmark.",
      "en-DK"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/dk/" },
      { name: "About", url: "https://carelabz.com/dk/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Denmark"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Denmark experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/dk/contact-us/"}
