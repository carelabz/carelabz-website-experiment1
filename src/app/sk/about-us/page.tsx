...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("sk");
  return {
    title: page?.metaTitle ?? "About Carelabs | Slovakian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and STN 33 2000 compliance across Slovakia.",
    alternates: {
      canonical: "https://carelabz.com/sk/about-us/",
      languages: {
        "en-SK": "https://carelabz.com/sk/about-us/",
        "x-default": "https://carelabz.com/sk/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Slovakia",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Slovakia.",
      url: "https://carelabz.com/sk/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SKAboutPage() {
  const page = await getAboutPage("sk");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Slovakia.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/sk/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Slovakia",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Slovakia.",
      "en-SK"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/sk/" },
      { name: "About", url: "https://carelabz.com/sk/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Slovakia"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Slovakian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/sk/contact-us/"}
