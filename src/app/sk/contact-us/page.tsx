...
const config = COUNTRY_CONFIGS["sk"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("sk");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Slovakia | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and STN 33 2000 compliance services across Slovakia.",
    alternates: {
      canonical: "https://carelabz.com/sk/contact-us/",
      languages: {
        "en-SK": "https://carelabz.com/sk/contact-us/",
        "x-default": "https://carelabz.com/sk/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Slovakia",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Slovakia.",
      url: "https://carelabz.com/sk/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SKContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "SK");
  // On the Slovakia contact page the banner must always say Slovakia
  // regardless of the visitor's IP — the services ARE in Slovakia.
  const countryName = "Slovakia";

  const [page, services] = await Promise.all([
    getContactPage("sk"),
    getServicesByRegion("sk"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Slovakian team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "sk", countryName: "Slovakia", countryIso2: "SK", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/sk/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Slovakia",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Slovakia.",
      "en-SK"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/sk/" },
      { name: "Contact", url: "https://carelabz.com/sk/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Slovakia Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Slovakian team is available to respond to emergency compliance and
