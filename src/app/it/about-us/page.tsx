...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("it");
  return {
    title: page?.metaTitle ?? "About Carelabs | Italy Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and CEI 64-8 compliance across Italy.",
    alternates: {
      canonical: "https://carelabz.com/it/about-us/",
      languages: {
        "en-IT": "https://carelabz.com/it/about-us/",
        "x-default": "https://carelabz.com/it/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Italy",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Italy.",
      url: "https://carelabz.com/it/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ITAboutPage() {
  const page = await getAboutPage("it");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Italy.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/it/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Italy",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Italy.",
      "en-IT"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/it/" },
      { name: "About", url: "https://carelabz.com/it/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Italy"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Italy experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/it/contact-us/"}
