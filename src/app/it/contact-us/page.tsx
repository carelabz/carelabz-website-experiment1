...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("it");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Italy | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and CEI 64-8 compliance services across Italy.",
    alternates: {
      canonical: "https://carelabz.com/it/contact-us/",
      languages: {
        "en-IT": "https://carelabz.com/it/contact-us/",
        "x-default": "https://carelabz.com/it/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Italy",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Italy.",
      url: "https://carelabz.com/it/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ITContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "IT");
  // On the Italy contact page the banner must always say Italy
  // regardless of the visitor's IP — the services ARE in Italy.
  const countryName = "Italy";

  const [page, services] = await Promise.all([
    getContactPage("it"),
    getServicesByRegion("it"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Italy team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/it/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Italy",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Italy.",
      "en-IT"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/it/" },
      { name: "Contact", url: "https://carelabz.com/it/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Italy Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Italy team is available to respond to emergency compliance and
