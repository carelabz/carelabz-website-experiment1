...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("eg");
  return {
    title: page?.metaTitle ?? "About Carelabs | Egypt Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and Egyptian Code for Electrical Installations compliance across Egypt.",
    alternates: {
      canonical: "https://carelabz.com/eg/about-us/",
      languages: {
        "en-EG": "https://carelabz.com/eg/about-us/",
        "x-default": "https://carelabz.com/eg/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Egypt",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Egypt.",
      url: "https://carelabz.com/eg/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function EGAboutPage() {
  const page = await getAboutPage("eg");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Egypt.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/eg/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Egypt",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Egypt.",
      "en-EG"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/eg/" },
      { name: "About", url: "https://carelabz.com/eg/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Egypt"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Egypt experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/eg/contact-us/"}
