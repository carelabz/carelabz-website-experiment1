...
const config = COUNTRY_CONFIGS["eg"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("eg");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Egypt | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and Egyptian Code for Electrical Installations compliance services across Egypt.",
    alternates: {
      canonical: "https://carelabz.com/eg/contact-us/",
      languages: {
        "en-EG": "https://carelabz.com/eg/contact-us/",
        "x-default": "https://carelabz.com/eg/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Egypt",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Egypt.",
      url: "https://carelabz.com/eg/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function EGContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "EG");
  // On the Egypt contact page the banner must always say Egypt
  // regardless of the visitor's IP — the services ARE in Egypt.
  const countryName = "Egypt";

  const [page, services] = await Promise.all([
    getContactPage("eg"),
    getServicesByRegion("eg"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Egypt team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "eg", countryName: "Egypt", countryIso2: "EG", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/eg/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Egypt",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Egypt.",
      "en-EG"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/eg/" },
      { name: "Contact", url: "https://carelabz.com/eg/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Egypt Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Egypt team is available to respond to emergency compliance and
