...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("ch");
  return {
    title: page?.metaTitle ?? "About Carelabs | Switzerland Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and NIN (SEV 1000) compliance across Switzerland.",
    alternates: {
      canonical: "https://carelabz.com/ch/about-us/",
      languages: {
        "en-CH": "https://carelabz.com/ch/about-us/",
        "x-default": "https://carelabz.com/ch/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Switzerland",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Switzerland.",
      url: "https://carelabz.com/ch/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function CHAboutPage() {
  const page = await getAboutPage("ch");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Switzerland.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/ch/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Switzerland",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Switzerland.",
      "en-CH"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ch/" },
      { name: "About", url: "https://carelabz.com/ch/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Switzerland"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Switzerland experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/ch/contact-us/"}
