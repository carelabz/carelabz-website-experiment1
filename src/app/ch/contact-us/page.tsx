...
const config = COUNTRY_CONFIGS["ch"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("ch");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Switzerland | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and NIN (SEV 1000) compliance services across Switzerland.",
    alternates: {
      canonical: "https://carelabz.com/ch/contact-us/",
      languages: {
        "en-CH": "https://carelabz.com/ch/contact-us/",
        "x-default": "https://carelabz.com/ch/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Switzerland",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Switzerland.",
      url: "https://carelabz.com/ch/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function CHContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "CH");
  // On the Switzerland contact page the banner must always say Switzerland
  // regardless of the visitor's IP — the services ARE in Switzerland.
  const countryName = "Switzerland";

  const [page, services] = await Promise.all([
    getContactPage("ch"),
    getServicesByRegion("ch"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Switzerland team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "ch", countryName: "Switzerland", countryIso2: "CH", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/ch/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Switzerland",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Switzerland.",
      "en-CH"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ch/" },
      { name: "Contact", url: "https://carelabz.com/ch/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Switzerland Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Switzerland team is available to respond to emergency compliance and
