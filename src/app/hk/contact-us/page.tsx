...
const config = COUNTRY_CONFIGS["hk"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("hk");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Hong Kong | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and CoP Electrical Installation compliance services across Hong Kong.",
    alternates: {
      canonical: "https://carelabz.com/hk/contact-us/",
      languages: {
        "en-HK": "https://carelabz.com/hk/contact-us/",
        "x-default": "https://carelabz.com/hk/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Hong Kong",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Hong Kong.",
      url: "https://carelabz.com/hk/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function HKContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "HK");
  // On the Hong Kong contact page the banner must always say Hong Kong
  // regardless of the visitor's IP — the services ARE in Hong Kong.
  const countryName = "Hong Kong";

  const [page, services] = await Promise.all([
    getContactPage("hk"),
    getServicesByRegion("hk"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Hong Kong team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "hk", countryName: "Hong Kong", countryIso2: "HK", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/hk/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Hong Kong",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Hong Kong.",
      "en-HK"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/hk/" },
      { name: "Contact", url: "https://carelabz.com/hk/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Hong Kong Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Hong Kong team is available to respond to emergency compliance and
