...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("hk");
  return {
    title: page?.metaTitle ?? "About Carelabs | Hong Kong Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and CoP Electrical Installation compliance across Hong Kong.",
    alternates: {
      canonical: "https://carelabz.com/hk/about-us/",
      languages: {
        "en-HK": "https://carelabz.com/hk/about-us/",
        "x-default": "https://carelabz.com/hk/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Hong Kong",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Hong Kong.",
      url: "https://carelabz.com/hk/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function HKAboutPage() {
  const page = await getAboutPage("hk");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Hong Kong.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/hk/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Hong Kong",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Hong Kong.",
      "en-HK"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/hk/" },
      { name: "About", url: "https://carelabz.com/hk/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Hong Kong"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Hong Kong experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/hk/contact-us/"}
