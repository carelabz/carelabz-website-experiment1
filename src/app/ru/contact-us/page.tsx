...
const config = COUNTRY_CONFIGS["ru"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("ru");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Russia | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and PUE-7 compliance services across Russia.",
    alternates: {
      canonical: "https://carelabz.com/ru/contact-us/",
      languages: {
        "en-RU": "https://carelabz.com/ru/contact-us/",
        "x-default": "https://carelabz.com/ru/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Russia",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Russia.",
      url: "https://carelabz.com/ru/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function RUContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "RU");
  // On the Russia contact page the banner must always say Russia
  // regardless of the visitor's IP — the services ARE in Russia.
  const countryName = "Russia";

  const [page, services] = await Promise.all([
    getContactPage("ru"),
    getServicesByRegion("ru"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Russian team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "ru", countryName: "Russia", countryIso2: "RU", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/ru/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Russia",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Russia.",
      "en-RU"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ru/" },
      { name: "Contact", url: "https://carelabz.com/ru/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Russia Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Russian team is available to respond to emergency compliance and
