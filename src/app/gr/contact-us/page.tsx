...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("gr");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Greece | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and ELOT HD 384 compliance services across Greece.",
    alternates: {
      canonical: "https://carelabz.com/gr/contact-us/",
      languages: {
        "en-GR": "https://carelabz.com/gr/contact-us/",
        "x-default": "https://carelabz.com/gr/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Greece",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Greece.",
      url: "https://carelabz.com/gr/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function GRContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "GR");
  // On the Greece contact page the banner must always say Greece
  // regardless of the visitor's IP — the services ARE in Greece.
  const countryName = "Greece";

  const [page, services] = await Promise.all([
    getContactPage("gr"),
    getServicesByRegion("gr"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Greece team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/gr/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Greece",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Greece.",
      "en-GR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/gr/" },
      { name: "Contact", url: "https://carelabz.com/gr/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Greece Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Greece team is available to respond to emergency compliance and
