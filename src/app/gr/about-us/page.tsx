...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("gr");
  return {
    title: page?.metaTitle ?? "About Carelabs | Greece Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and ELOT HD 384 compliance across Greece.",
    alternates: {
      canonical: "https://carelabz.com/gr/about-us/",
      languages: {
        "en-GR": "https://carelabz.com/gr/about-us/",
        "x-default": "https://carelabz.com/gr/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Greece",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Greece.",
      url: "https://carelabz.com/gr/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function GRAboutPage() {
  const page = await getAboutPage("gr");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Greece.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/gr/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Greece",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Greece.",
      "en-GR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/gr/" },
      { name: "About", url: "https://carelabz.com/gr/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Greece"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Greece experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/gr/contact-us/"}
