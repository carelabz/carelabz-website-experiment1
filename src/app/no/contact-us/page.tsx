...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("no");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Norway | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and NEK 400 compliance services across Norway.",
    alternates: {
      canonical: "https://carelabz.com/no/contact-us/",
      languages: {
        "en-NO": "https://carelabz.com/no/contact-us/",
        "x-default": "https://carelabz.com/no/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Norway",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Norway.",
      url: "https://carelabz.com/no/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function NOContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "NO");
  // On the Norway contact page the banner must always say Norway
  // regardless of the visitor's IP — the services ARE in Norway.
  const countryName = "Norway";

  const [page, services] = await Promise.all([
    getContactPage("no"),
    getServicesByRegion("no"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Norway team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/no/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Norway",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Norway.",
      "en-NO"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/no/" },
      { name: "Contact", url: "https://carelabz.com/no/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Norway Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Norway team is available to respond to emergency compliance and
