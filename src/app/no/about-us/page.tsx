...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("no");
  return {
    title: page?.metaTitle ?? "About Carelabs | Norway Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and NEK 400 compliance across Norway.",
    alternates: {
      canonical: "https://carelabz.com/no/about-us/",
      languages: {
        "en-NO": "https://carelabz.com/no/about-us/",
        "x-default": "https://carelabz.com/no/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Norway",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Norway.",
      url: "https://carelabz.com/no/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function NOAboutPage() {
  const page = await getAboutPage("no");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Norway.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/no/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Norway",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Norway.",
      "en-NO"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/no/" },
      { name: "About", url: "https://carelabz.com/no/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Norway"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Norway experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/no/contact-us/"}
