...
const config = COUNTRY_CONFIGS["se"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("se");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Sweden | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and EBR 18:2017 compliance services across Sweden.",
    alternates: {
      canonical: "https://carelabz.com/se/contact-us/",
      languages: {
        "en-SE": "https://carelabz.com/se/contact-us/",
        "x-default": "https://carelabz.com/se/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Sweden",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Sweden.",
      url: "https://carelabz.com/se/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SEContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "SE");
  // On the Sweden contact page the banner must always say Sweden
  // regardless of the visitor's IP — the services ARE in Sweden.
  const countryName = "Sweden";

  const [page, services] = await Promise.all([
    getContactPage("se"),
    getServicesByRegion("se"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Sweden team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "se", countryName: "Sweden", countryIso2: "SE", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/se/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Sweden",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Sweden.",
      "en-SE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/se/" },
      { name: "Contact", url: "https://carelabz.com/se/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Sweden Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Sweden team is available to respond to emergency compliance and
