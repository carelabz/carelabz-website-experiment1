...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("se");
  return {
    title: page?.metaTitle ?? "About Carelabs | Sweden Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and EBR 18:2017 compliance across Sweden.",
    alternates: {
      canonical: "https://carelabz.com/se/about-us/",
      languages: {
        "en-SE": "https://carelabz.com/se/about-us/",
        "x-default": "https://carelabz.com/se/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Sweden",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Sweden.",
      url: "https://carelabz.com/se/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function SEAboutPage() {
  const page = await getAboutPage("se");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Sweden.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/se/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Sweden",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Sweden.",
      "en-SE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/se/" },
      { name: "About", url: "https://carelabz.com/se/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Sweden"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Sweden experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/se/contact-us/"}
