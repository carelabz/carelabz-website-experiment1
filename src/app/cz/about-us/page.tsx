...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("cz");
  return {
    title: page?.metaTitle ?? "About Carelabs | Czech Republic Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and ČSN 33 2000 compliance across Czech Republic.",
    alternates: {
      canonical: "https://carelabz.com/cz/about-us/",
      languages: {
        "en-CZ": "https://carelabz.com/cz/about-us/",
        "x-default": "https://carelabz.com/cz/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Czech Republic",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Czech Republic.",
      url: "https://carelabz.com/cz/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function CZAboutPage() {
  const page = await getAboutPage("cz");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Czech Republic.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/cz/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Czech Republic",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Czech Republic.",
      "en-CZ"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/cz/" },
      { name: "About", url: "https://carelabz.com/cz/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Czech Republic"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Czech Republic experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/cz/contact-us/"}
