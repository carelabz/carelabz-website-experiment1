...
const config = COUNTRY_CONFIGS["cz"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("cz");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Czech Republic | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and ČSN 33 2000 compliance services across Czech Republic.",
    alternates: {
      canonical: "https://carelabz.com/cz/contact-us/",
      languages: {
        "en-CZ": "https://carelabz.com/cz/contact-us/",
        "x-default": "https://carelabz.com/cz/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Czech Republic",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Czech Republic.",
      url: "https://carelabz.com/cz/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function CZContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "CZ");
  // On the Czech Republic contact page the banner must always say Czech Republic
  // regardless of the visitor's IP — the services ARE in Czech Republic.
  const countryName = "Czech Republic";

  const [page, services] = await Promise.all([
    getContactPage("cz"),
    getServicesByRegion("cz"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Czech Republic team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "cz", countryName: "Czech Republic", countryIso2: "CZ", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/cz/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Czech Republic",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Czech Republic.",
      "en-CZ"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/cz/" },
      { name: "Contact", url: "https://carelabz.com/cz/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Czech Republic Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Czech Republic team is available to respond to emergency compliance and
