...
const config = COUNTRY_CONFIGS["pt"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("pt");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Portugal | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and RTIEBT compliance services across Portugal.",
    alternates: {
      canonical: "https://carelabz.com/pt/contact-us/",
      languages: {
        "en-PT": "https://carelabz.com/pt/contact-us/",
        "x-default": "https://carelabz.com/pt/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Portugal",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Portugal.",
      url: "https://carelabz.com/pt/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PTContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "PT");
  // On the Portugal contact page the banner must always say Portugal
  // regardless of the visitor's IP — the services ARE in Portugal.
  const countryName = "Portugal";

  const [page, services] = await Promise.all([
    getContactPage("pt"),
    getServicesByRegion("pt"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Portugal team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "pt", countryName: "Portugal", countryIso2: "PT", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/pt/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Portugal",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Portugal.",
      "en-PT"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/pt/" },
      { name: "Contact", url: "https://carelabz.com/pt/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Portugal Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Portugal team is available to respond to emergency compliance and
