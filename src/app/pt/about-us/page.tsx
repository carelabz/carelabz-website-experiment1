...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("pt");
  return {
    title: page?.metaTitle ?? "About Carelabs | Portugal Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and RTIEBT compliance across Portugal.",
    alternates: {
      canonical: "https://carelabz.com/pt/about-us/",
      languages: {
        "en-PT": "https://carelabz.com/pt/about-us/",
        "x-default": "https://carelabz.com/pt/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Portugal",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Portugal.",
      url: "https://carelabz.com/pt/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PTAboutPage() {
  const page = await getAboutPage("pt");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Portugal.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/pt/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Portugal",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Portugal.",
      "en-PT"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/pt/" },
      { name: "About", url: "https://carelabz.com/pt/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Portugal"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Portugal experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/pt/contact-us/"}
